...
disease_info = {
    # "Tomato___Late_blight": "Là một bệnh nấm gây ra các đốm nâu trên lá..."
}
...
if image_source is not None:
    ...
    with col2:
        ...
        if confidence < CONFIDENCE_THRESHOLD:
            st.markdown(
                f"<div class='result-box' style='background-color: #f8d7da; color: #842029; border-color: #f5c2c7;'>⚠️ Ảnh không hợp lệ hoặc không rõ là lá cây.<br/>🎯 Độ chính xác quá thấp: {confidence:.2f}%</div>",
                unsafe_allow_html=True
            )
        else:
            display_class = predicted_class.replace("Tomato___", "").replace("_", " ")
            st.markdown(
                f"<div class='result-box'>✅ Kết quả dự đoán: <strong>{display_class}</strong><br/>🎯 Độ chính xác: {confidence:.2f}%</div>",
                unsafe_allow_html=True
            )
